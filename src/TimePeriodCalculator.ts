...
class TimePeriodCalculator {
    ...
    public getTimePeriod(data: string): moment.Duration {
        ...
        let clockPattern = '\\d{2}:\\d{2}(?::\\d{2}(?:[.,]\\d{3,})?)?(?:Z| ?[+-]\\d{2}:\\d{2}| ?[+-]\\d{4})?\\b';

        // ISO dates ("2016-08-23")
        const isoDatePattern = '^\\d{4}-\\d{2}-\\d{2}(?:T|\\b)';

        // Data especifica para log progress ("19/05/22")
        const progressDatePattern = '^[\[]\\d{2}[^\\w\\s]\\d{2}[^\\w\\s]\\d{2}\\b';
        ...
        const dateTimePattern = '((?:' + isoDatePattern + '|' + cultureDatesPattern + '|' + progressDatePattern + '){1}?[ @]?' +
            '(?:' + clockPattern + '){1})';
        ...
        const datesPattern = '(' + isoDatePattern + '|' + cultureDatesPattern + '|' + progressDatePattern + '){1}';
